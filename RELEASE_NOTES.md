...
Phiên bản này được đánh dấu là sẵn sàng cho việc chạy thử nội bộ/cục bộ.
Yêu cầu & Ràng buộc cho môi trường chạy thử:
- Không deploy public internet.
- Không nhập dữ liệu pháp lý thật, chỉ sử dụng dữ liệu giả hoặc dữ liệu đã ẩn danh.
- Không upload tài liệu thật.
- SQLite hiện tại chỉ phục vụ cho MVP và Local Trial.

**Các tính năng hoàn thiện sẵn sàng cho Trial:**
- Đã vượt qua kiểm thử UAT tổng thể v0.9.0 thành công 100%.
- Cấu trúc bảo mật được nâng cấp (Security headers, Rate limiting).
- Đầy đủ tính năng quản trị người dùng (User Management).
- Phân quyền theo Role-Based Access Control (RBAC).
- Ghi log hệ thống an toàn (Admin Audit Logs).
- Cơ chế sao lưu/khôi phục an toàn nhiều lớp (Backup/Restore).
...
## v0.9.0-security-hardening (22/05/2026)  
  
### T�nh nang b?o m?t (Phase 5)  
- **Security Headers**: B?t Helmet cho backend, t?i uu CORS cho localhost.  
- **Rate Limiting**: Ap d?ng Global limit (60 req/min) v� Endpoint nh?y c?m (login, change-password, reset-password - 5 req/min).  
- **Input Hardening**: T?i uu h�a ValidationPipe t? ch?i c�c tham s? r�c (forbidNonWhitelisted).  
- **Audit Logging Auth**: T? d?ng log h�nh vi LOGIN_SUCCESS v� LOGIN_FAILED v�o h? th?ng.  
- **Password Policy**: Quy ho?ch ch�nh s�ch m?t kh?u t?p trung d�ng chung cho to�n b? endpoint.  
  
### Quy?t d?nh thi?t k?  
- 429 Rate Limit du?c tr? v? v?i message th�n thi?n ti?ng Vi?t, kh�ng kh�a c?ng DB.  
- S? ki?n RATE_LIMIT_HIT ch? log ra server console d? ti?t ki?m dung lu?ng luu tr?. 
