...
export class AnalyticsService {
  ...
  async getByNeighborhood(query: any) {
    ...
    return data.map((item) => ({
      ...
      percentage:
        totalCases > 0 ? ((item._count.id / totalCases) * 100).toFixed(2) : 0,
    }));
  }
  ...
  async getCrossTab(query: any) {
    ...
    const data = await this.prisma.legalCase.groupBy({
      ...
      where,
      _count: { id: true },
      orderBy: [{ neighborhood: 'asc' }, { field: 'asc' }],
    });
  ...
  async getSocialInsights(query: any) {
    ...
    if (topNeighborhoods.length > 0) {
      ...
      observations.push(
        `Khu phố ${topN.neighborhood} ghi nhận số lượng đơn cao nhất (${topN._count.id} đơn).`,
      );
      hypotheses.push(
        `Số lượng đơn cao tại khu phố ${topN.neighborhood} có thể phản ánh một số yếu tố khách quan như mật độ dân cư tập trung, sự chủ động tiếp cận kênh phản ánh của người dân, hoặc các biến động cục bộ đang diễn ra. Cần đối chiếu thêm với dữ liệu dân số thực tế để đánh giá tỷ lệ tương đối.`,
      );
    }
    ...
    if (topFields.length > 0) {
      ...
      observations.push(
        `Lĩnh vực ${topF.field} chiếm số lượng lớn nhất trong tổng số đơn thư (${topF._count.id} đơn).`,
      );
      hypotheses.push(
        `Sự tập trung đơn thư vào lĩnh vực ${topF.field} gợi ý rằng đây có thể là vấn đề đang nhận được nhiều sự quan tâm hoặc áp lực xã hội trong cộng đồng. Điều này thường xuất hiện khi có sự thay đổi về chính sách, quy hoạch hoặc các tác động kinh tế - xã hội liên quan. Nên ưu tiên nguồn lực để giải thích, hỗ trợ pháp lý hoặc giải quyết dứt điểm các vướng mắc trong mảng này.`,
      );
    }
